...
| **多 API 切换** | 独立保存多套接口、模型与密钥，在浮窗顶部直接切换；所有已打开页面同步当前选择。 |
...
| 配置项 | 填写说明 |
| --- | --- |
| **配置名称** | 给每套配置起一个易识别的名字，例如「日常问答」「代码助手」。 |
...
| **图片输入** | 仅在所选模型支持图片时勾选。 |
| **API 密钥** | 每套配置独立保存密钥；编辑同一配置且 Base URL 未变时，留空可保留原密钥；修改 Base URL 后需要重新填写。 |

远程接口必须使用 HTTPS，本机的 `localhost` 和 `127.0.0.1` 可以使用 HTTP。API 密钥保存在本机的扩展存储中，扩展重新加载或 Chrome 重启后仍保留，保存后不回显；可在设置页点击 **忘记此配置的 API 密钥** 移除。

设置页点击 **新增配置** 可保存更多 API；首个配置自动启用，后续新增或编辑仅保存配置。点击列表中的 **使用**，或在聊天浮窗顶部的 **API** 下拉框中切换当前配置。**测试连接** 始终测试当前编辑且已保存的配置。

当前选择在所有 ChatGPT 页面间同步，并在刷新后保留。切换保留当前侧边历史、引用和输入草稿；已经提交的问题（包括附件确认中或正在生成的回答）继续使用提交时的配置，下一条消息使用新选择。旧版本保存的单套 API 和密钥会自动迁移为「默认 API」。
...
| **切换 API** | 在浮窗顶部选择已保存的配置，下一条消息生效；继续使用当前侧边对话历史。 |
