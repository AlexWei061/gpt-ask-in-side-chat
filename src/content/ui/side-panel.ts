...
export class SidePanel {
  ...
  private busy = false; private accepted = false; private stream = ""; private resizing = false;
  constructor(private readonly document: Document, private readonly options: SidePanelOptions) {
    ...
    document.documentElement.append(this.host); this.render();
  }
  setWidth(width: number): void { this.width = this.clamp(width); this.render(); this.applyMargin(); }
  setConversation(id: string | null, messages: SideMessage[]): void { this.conversationId = id; this.messages = messages; this.quote = null; this.draft = ""; this.error = null; this.lastSubmission = null; this.busy = false; this.accepted = false; this.stream = ""; this.render(); }
  setMessages(messages: SideMessage[]): void { this.messages = messages; this.stream = ""; this.render(); }
  open(quote: QuoteReference): void { this.quote = quote; if (this.originalMargin === null) this.originalMargin = this.document.body.style.marginRight; this.applyMargin(); this.render(); this.root.querySelector<HTMLTextAreaElement>("textarea")?.focus(); }
  close(): void { this.quote = null; this.restoreMargin(); this.render(); }
  setError(error: PanelError | null): void { this.error = error; this.busy = false; this.render(); }
  ...
  private applyMargin(): void { if (this.quote) this.document.body.style.marginRight = `${this.width}px`; }
  ...
  private render(): void {
    ...
    const resize = this.document.createElement("div"); resize.className = "resize"; resize.dataset.resizeHandle = "true"; resize.setAttribute("aria-label", "Resize side chat"); resize.addEventListener("pointerdown", this.down); panel.append(resize);
    ...
    const status = this.document.createElement("div"); status.className = "status"; if (this.error) { status.textContent = this.error.message; if (this.error.retryable && this.lastSubmission) status.append(this.button("Retry", "retry", () => this.send(this.lastSubmission!))); } else if (this.busy) status.textContent = "Generating…"; panel.append(status);
  ...
  private send(payload: PanelSend): void { if (this.busy) return; this.lastSubmission = payload; this.error = null; this.busy = true; this.options.onSend(payload); this.render(); }
